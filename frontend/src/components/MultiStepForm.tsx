...
export const MultiStepForm: React.FC = () => {
    ...
    function validateDocuments(filesRaw: File[] | undefined): FieldErrors {
        ...
        for (let i = 0; i < arr.length; i += 1) {
            ...
            if (f.size > MAX_FILE_SIZE) {
                e.files = `File "${f.name}" exceeds ${Math.round(MAX_FILE_SIZE / 1024 / 1024)}MB`;
                return e;
            }
        }
        return e;
        ...
        const uploadedFiles = await Promise.all(
            filesRaw.slice(0, MAX_FILES).map(async (file) => {
                ...
                return {
                    ...
                    filename: file.name,
                    data: base64Data.split(",")[1], // strip "data:...;base64,"
                    mimeType: file.type,
                    size: file.size,
                };
            })
    ...
    async function onSubmit(e?: React.FormEvent<HTMLFormElement>): Promise<void> {
        ...
        if (isIntentionalSubmit) {
            try {
                ...
                const message = err instanceof Error ? err.message : "Failed to build payload";
    ...
    const renderSubmitError = (): JSX.Element | null => {
        ...
        if (mutation.isSuccess) {
            return <div style={{ color: "green", marginTop: 8 }}>Submitted! ID: {mutation.data?.id}</div>;
        }
        return null;
    };
    ...
    return (
        <div className="container">
            ...
            >
                ...
                {step === 3 && (
                    <>
                        <Step3Documents data={data} update={update} updateFiles={updateFilesRaw} />
                        <div style={{ color: "red", fontSize: 13 }}>
                ...
                <div className="actions" style={{ marginTop: 8 }}>
                    {step > 1 && (<button type="button" onClick={prev} disabled={step === 1}>
                        Back
                    </button>)}
